...
from networks.resnet_big import Bottleneck as CustomBottleneck
from networks.resnet_big import BasicBlock as CustomBasicBlock
from networks.resnet_big import ResNet as CustomResNet

# from avalanche based code, commented out
# from resnet import Bottleneck as CustomBottleneck
# from resnet import BasicBlock as CustomBasicBlock
# from resnet import ResNet as CustomResNet

from torchray.attribution.common import Patch, Probe
from torchray.attribution.common import attach_debug_probes, get_backward_gradient, get_module
from torchray.attribution.common import saliency, resize_saliency
...
def update_resnet(model, debug=False):
    ...
    """
    assert isinstance(model, ResNet) or isinstance(model, CustomResNet)
    ...
    for module_name, module in model.named_modules():
        if isinstance(module, Bottleneck) or isinstance(module, CustomBottleneck):
            module.skip = EltwiseSum()
            module.forward = bottleneck_forward.__get__(module)
        elif isinstance(module, BasicBlock) or isinstance(module, CustomBasicBlock):
            module.skip = EltwiseSum()
    ...
    def _patch_conv(target_name, enable, debug):
        ...
        def backward(ctx, grad_output):
            ...
            with torch.enable_grad():
                ...
                # them to None?
                #
                # [XXXXX: No! we will set them to be the geometric mean of
                ...
                eb_output = grad_output.detach().clone().requires_grad_(True)
                if enable:
                    grad_output = grad_output / (output_ebp + 1e-20)

                # 4. do backward pass.
                _ = torch.autograd.grad(
                    output_ebp, inputs_subset, grad_output, only_inputs=True, allow_unused=True)
                for i, j in enumerate(subset):
                    grad_inputs_[j] = _[i]

                # 5. multiply gradient with the layer's input.
                if ctx.needs_input_grad[0] and enable:
                    # added the below line to correct numerical errors resulting in negative eb values
                ...
                # forward
                eb_input = grad_inputs_[0].detach().clone().requires_grad_(True)
                # assert (eb_input < 0).sum() == 0
                # assert (eb_output < 0).sum() == 0
                ...
                grad_inputs_subset = [eb_input, weight_clone, inputs_[2]]
                grad_inputs_subset = [grad_inputs_subset[i] for i in subset]
                # backward
                _w = torch.autograd.grad(
                    output_ebp_w, grad_inputs_subset, eb_output, only_inputs=True, allow_unused=True)
                for i, j in enumerate(subset):
                    grad_inputs_w[j] = _w[i]
                # sqrt for geometric mean (clamp only to correct numerical errors)
                grad_inputs_w[1] = torch.sqrt(torch.clamp(grad_inputs_w[1], min=0))
                # assert torch.isnan(grad_inputs_w[1]).sum() == 0

                # bias gradient will count the number of times an operation's gradients
                # have been accumulated
                if inputs_[2] is not None:
                    grad_inputs_[2] = torch.ones_like(inputs_[2])

                return (grad_inputs_[0], grad_inputs_w[1], grad_inputs_[2],
                        None, None, None, None)
    ...
    def __enter__(self):
        ...
        self.patches = [
            ...
            self._patch_norm('torch.nn.functional.batch_norm',
                             self.enable, self.debug),
            self._patch_norm('torch.nn.functional.normalize',
                             self.enable, self.debug),
